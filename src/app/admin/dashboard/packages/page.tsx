...
export default function AdminPackages() {
  ...
  const [category, setCategory] = useState<"internet_only" | "internet_tv">("internet_only");
  ...
  const openFormModal = (pkg?: PackageRow) => {
    if (pkg) {
      ...
      setName(pkg.name);
      setCategory(pkg.category || "internet_only");
      setSpeed(pkg.speed);
      setPrice(pkg.price);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      let result;
      if (editingPackage) {
        result = await updatePackage(editingPackage.id, { 
          name, category, speed, price, description, active, features: cleanedFeatures, is_popular: isPopular
        });
      } else {
      ...
      if (result && result.replacedPopular) {
        alert("Previous popular package has been replaced.");
      }
              ...
              {packages.map((pkg) => (
                <tr key={pkg.id} className="hover:bg-slate-50 transition">
                  ...
                  <td className="px-6 py-4">
                    {pkg.category === "internet_tv" ? (
                      <span className="px-2.5 py-1 rounded-full text-xs font-bold inline-flex items-center bg-purple-100 text-purple-700 border border-purple-200">
                        Internet + TV
                      </span>
                    ) : (
                      <span className="px-2.5 py-1 rounded-full text-xs font-bold inline-flex items-center bg-blue-100 text-blue-700 border border-blue-200">
                        Internet Only
                      </span>
                    )}
                  </td>
                  ...
                  <td className="px-6 py-4">
                    {pkg.is_popular ? (
                      <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-amber-100 text-amber-700 text-xs font-bold uppercase tracking-wider">
            ...
            <form onSubmit={handleSave} className="flex-1 overflow-y-auto p-6 space-y-5">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="col-span-2">
                  ...
                  <select
                    className="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-brand-500 bg-white"
                    value={category}
                    onChange={(e) => setCategory(e.target.value as "internet_only" | "internet_tv")}
                  >
